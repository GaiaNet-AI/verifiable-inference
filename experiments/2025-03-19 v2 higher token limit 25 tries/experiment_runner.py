...
AI inference models running locally, with automatic model availability checking.
"""
...
class ExperimentRunner:
    ...
    def run_sequential_experiment(self):
        """
        Run the experiment sequentially for each model with automatic availability checking.
        
        Returns:
            Dict: The raw results from the experiment
        """
        ...
        for model_name, model_url in self.config.MODELS.items():
            ...
            for q_idx, question in enumerate(self.config.QUESTIONS):
                q_key = f"Q{q_idx+1}"
                logger.info(f"Processing {model_name}, {q_key}")
                ...
                model_results[q_key] = question_results
